...
// HPB+
int callback_hpb(void *NotUsed, int argc, char **argv, char **azColName)
{
	int i = 0;
	NotUsed = 0;
	for (i = 0; i < argc; i++)
		printf("[HPB+ SELECT] %s = %s\n", azColName[i], argv[i] ? argv[i] : "NULL");
	...
	for(tx_i = 0; tx_i < db_transactions; tx_i++) 
	{
		...
		else if(db_mode == 3) // SELECT
		{
			for(db_i = 0; db_i<numberOfDB; db_i++)
			{
				...
				{
					if (db_i == 0)
						length += sprintf(sql+length, "SELECT * from tblMyList%d WHERE id=%d;", tbl_i, rand()%column_count+1);
					else
						length += sprintf(sql+length, "SELECT * from db%d.tblMyList%d WHERE id=%d;", db_i, tbl_i, rand()%column_count+1);
				}
			}
			...
			exec_sql(db[0],sql,NULL);
			//exec_sql(db[0],sql,callback_hpb);
...
"           -d  enable DB test mode (0=insert, 1=update, 2=delete 3=select)",
