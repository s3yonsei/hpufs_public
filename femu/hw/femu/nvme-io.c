...
static void nvme_process_cq_hpb(void *arg)
{
    FemuCtrl *n = (FemuCtrl *)arg;
    NvmeCQueue *cq = NULL;
    NvmeRequest *req = NULL;
    struct rte_ring *rp = n->to_hpb_poller[1];
    pqueue_t *hpb_pq = n->hpb_pq[1];
    hwaddr addr;
    uint64_t now;
    int rc;
    ...
    while (femu_ring_count(rp)) {
        ...
        assert(req);
        pqueue_insert(hpb_pq, req);
    }

    while ((req = pqueue_peek(hpb_pq))) {
        ...
        req->cqe.cid = req->cmd.cid;
        req->cqe.status = cpu_to_le16(0 << 1 | cq->phase);
        req->cqe.sq_id = cpu_to_le16(req->sq->sqid);
        req->cqe.sq_head = cpu_to_le16(req->sq->head);

#ifdef HPB_PLUS
        ssd_write_segment_request(n, req);
        ssd_multi_update_l2p_request(n, req);
        ssd_transfer_l2p_entry(n, req);
        ssd_transfer_gc_vpb_request(n, req);
#endif
        ...
        nvme_addr_write(n, addr, (void *)&req->cqe, sizeof(req->cqe));
        nvme_inc_cq_tail(cq);
...
void nvme_create_poller(FemuCtrl *n)
{
    ...
    n->to_hpb = malloc(sizeof(struct rte_ring *) * (n->num_poller + 1));
    n->to_hpb_poller = malloc(sizeof(struct rte_ring *) * (n->num_poller + 1));
    n->hpb_pq = malloc(sizeof(pqueue_t *) * (n->num_poller + 1));
    
    for (int i = 1; i <= n->num_poller; i++) {
        n->to_hpb[i] = femu_ring_create(FEMU_RING_TYPE_MP_SC, FEMU_MAX_INF_REQS);
        if (!n->to_hpb[i]) {
            femu_err("failed to create ring (n->to_hpb) ...\n");
            abort();
        }
        assert(rte_ring_empty(n->to_hpb[i]));
    }

    for (int i = 1; i <= n->num_poller; i++) {
        n->to_hpb_poller[i] = femu_ring_create(FEMU_RING_TYPE_MP_SC, FEMU_MAX_INF_REQS);
        if (!n->to_hpb_poller[i]) {
            femu_err("failed to create ring (n->to_hpb_poller) ...\n");
            abort();
        }
        assert(rte_ring_empty(n->to_hpb_poller[i]));
    }

    for (int i = 1; i <= n->num_poller; i++) {
        n->hpb_pq[i] = pqueue_init(FEMU_MAX_INF_REQS, cmp_pri, get_pri, set_pri,
                               get_pos, set_pos);
        if (!n->hpb_pq[i]) {
            femu_err("failed to create pqueue (n->hpb_pq) ...\n");
            abort();
        }
    }
...
uint16_t nvme_rw(FemuCtrl *n, NvmeNamespace *ns, NvmeCmd *cmd, NvmeRequest *req)
{
    ...
    if ((rw->opcode == NVME_ADM_HPB_READ_SEGMENT) ||
        (rw->opcode == NVME_ADM_HPB_TRANSFER_SEGMENT))
    {
        slba += (n->total_l2p_segment_in_table * 1024);
    ...
    else if (rw->opcode == NVME_ADM_HPB_TRANSFER_SEGMENT)
    {
        req->is_write = 1;
    }
#endif
...
#ifdef HPB_PLUS
    if (rw->opcode == NVME_ADM_HPB_WRITE_SEGMENT)
    {
        slba += (n->total_l2p_segment_in_table * 1024);
    }
    else if (rw->opcode == NVME_ADM_HPB_MULTI_L2P_ENTRY) {
        slba = (n->total_l2p_segment_in_table * 1024) + n->total_l2p_segment_in_table;
    }
    else if (rw->opcode == NVME_ADM_HPB_TRANSFER_VPB) {
        slba = (n->total_l2p_segment_in_table * 1024) + n->total_l2p_segment_in_table + 6;
    }
    data_offset = slba << data_shift;

    if (rw->opcode == NVME_ADM_HPB_MULTI_L2P_ENTRY)
    {
        req->is_write = 0;
    }
    else if ((rw->opcode == NVME_ADM_HPB_WRITE_SEGMENT) ||
             (rw->opcode == NVME_ADM_HPB_TRANSFER_VPB))
    {
        req->is_write = 1;
    }
#endif
    ...
    */
    ...
    req->slba = slba;
    req->ppa = (uint32_t)(le64_to_cpu(rw->rsvd2) >> 32);
    req->key = (uint32_t)le64_to_cpu(rw->rsvd2);

    req->control = le16_to_cpu(rw->control);
    ...
    if ((rw->opcode == NVME_ADM_HPB_READ_SEGMENT) || 
        (rw->opcode == NVME_ADM_HPB_TRANSFER_SEGMENT))
    {
        req->slba = slba - (n->total_l2p_segment_in_table * 1024);
    }

    if (rw->opcode == NVME_ADM_HPB_TRANSFER_SEGMENT) 
    {
        if (req->key != n->ssd->l2p_table.arr_segment_sequence_num[req->slba]) 
        {
            return NVME_SUCCESS;
        }
    }
    ...
    else if (rw->opcode == NVME_CMD_WRITE)
    {
        //printf("write %ld %d\n", req->slba, req->nlb);
    }

#ifdef HPB_PLUS
    req->complete_sequence_number = (uint32_t)le64_to_cpu(rw->rsvd2);
    ...
    if (rw->opcode == NVME_ADM_HPB_WRITE_SEGMENT)
    {
        ...
        req->slba = 0;
        ssd_multi_update_set_queue_entry(n, req);
    }
    else if (rw->opcode == NVME_ADM_HPB_TRANSFER_VPB)
    {
        req->slba = le64_to_cpu(rw->slba);
    }
    ssd_complete_sequence_number(n, req, rw);
#endif
