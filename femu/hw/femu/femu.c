...
static void femu_realize(PCIDevice *pci_dev, Error **errp)
{
    ...
    n->blk_per_plane = 9400;
    n->meta_blk_per_plane =  ((int64_t)n->memsz) / 16 / 64;
    n->total_l2p_segment_in_table = ((int64_t)n->memsz) / 4;

    init_dram_backend(&n->mbe, bs_size + meta_size + (16 * 1536) + (4 * 1024));
...
static void nvme_destroy_poller(FemuCtrl *n)
{
    ...
    for (int i = 1; i <= n->num_poller; i++) {
        pqueue_free(n->hpb_pq[i]);
        femu_ring_free(n->to_hpb_poller[i]);
        femu_ring_free(n->to_hpb[i]);
    }
...
static Property femu_props[] = {
    ...
    DEFINE_PROP_UINT32("entries", FemuCtrl, max_q_ents, 0x10),
    DEFINE_PROP_UINT8("multipoller_enabled", FemuCtrl, multipoller_enabled, 0),
    DEFINE_PROP_UINT8("max_cqes", FemuCtrl, max_cqes, 0x5),
    ...
    DEFINE_PROP_UINT8("mdts", FemuCtrl, mdts, 5),
    DEFINE_PROP_UINT8("cqr", FemuCtrl, cqr, 1),
    DEFINE_PROP_UINT8("vwc", FemuCtrl, vwc, 1),
