...
static uint16_t nvme_admin_cmd(FemuCtrl *n, NvmeCmd *cmd, NvmeCqe *cqe)
{
    ...
    case NVME_ADM_CMD_SECURITY_RECV:
        return NVME_INVALID_OPCODE | NVME_DNR;
    case NVME_ADM_HPB_READ_SEGMENT:
    case NVME_ADM_HPB_TRANSFER_SEGMENT:
    case NVME_ADM_HPB_MULTI_L2P_ENTRY:
    case NVME_ADM_HPB_WRITE_SEGMENT:
    case NVME_ADM_HPB_TRANSFER_VPB:
        return NVME_HPB_CMD;
    default:
...
void nvme_process_sq_admin(void *opaque)
{
    ...
    while (!(nvme_sq_empty(sq))) {
        ...
        if ((n->dataplane_started) && (status == NVME_HPB_CMD))
        {
            ...
            memcpy(&req->cqe, &cqe, sizeof(req->cqe));
            ...
            int rc = femu_ring_enqueue(n->to_hpb[1], (void *)&req, 1);
        ...
        else
        {
            ...
            cqe.n.rsvd = 0;
#ifdef HPB_PLUS
            cqe.n.rsvd = NVME_HPB_PLUS_ENABLE << 30;
            cqe.n.rsvd |= TOTAL_CACHE_SEGMENT << 14;
            cqe.n.rsvd |= (n->blk_per_plane & 0x3FFF);
#else
            cqe.n.rsvd = NVME_HPB_ENABLE << 30;
            cqe.n.rsvd |= TOTAL_CACHE_SEGMENT << 14;
            cqe.n.rsvd |= (n->blk_per_plane & 0x3FFF);
#endif
#endif
